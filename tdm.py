...
class TDM():
    ...
    def fit_transform(self, X, verbose=True, report_interval=500, X_true=None):
        ...
        if not is_no_missing:
            # print(self.noise)
            # print(torch.randn(mask.shape))
            # print(nanmean(X, 0))
            # print(mask.bool())
            
            imps = (self.noise * torch.randn(mask.shape, device=X.device) + nanmean(X, 0))[mask.bool()]
            # print(imps)
        ...
        for i in range(self.niter):
            ...
            for _ in range(self.n_pairs):
                ...
                #idx1 = torch.tensor(idx1, device=X.device) 
                #idx2 = torch.tensor(idx2, device=X.device) 

                X1 = X_filled[idx1].cpu()
                X2 = X_filled[idx2].cpu()
